...
export default function ConfirmationDialog({
  ...
  // Escape key close handler and focus trapping
  useEffect(() => {
    if (!isOpen) return
  ...
  return (
    <div className="fixed inset-0 z-55 flex items-center justify-center px-4 py-6" role="dialog" aria-modal="true" aria-labelledby="dialog-title">
      {/* Backdrop */}
      <div onClick={onCancel} className="absolute inset-0 bg-gray-900/50 backdrop-blur-xs transition-opacity" />

      {/* Panel */}
      <div ref={dialogRef} className="relative w-full max-w-sm bg-white rounded-xl border border-gray-250 shadow-2xl p-5 z-55 flex flex-col space-y-4 animate-scaleUp">
